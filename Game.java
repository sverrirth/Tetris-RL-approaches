...
public class Game {
	...
	private static double[] stateValues = new double[(int)(Math.pow(2, width*2))];
	...
	private static final int totalGames = 1;
	...
	public boolean[][] copyWell(boolean[][] w) {
	...
	public boolean[][] removeBottomRow(boolean[][] w) {
		boolean[][] ret = new boolean[width][2];
		for(int i=0;i<width;i++) {
			ret[i][0] = w[i][1];
		}
	...
	public boolean[][] removeTopRow(boolean[][] w) {
		boolean[][] ret = new boolean[width][2];
		for(int i=0;i<width;i++) {
			ret[i][0] = w[i][0];
		}
	...
	public boolean[][] clean(boolean[][] w) {
		...
		if(bottomRowFlag && topRowFlag) return new boolean[width][2];
		else if(bottomRowFlag) return removeBottomRow(w);
		else if(topRowFlag) return removeTopRow(w);
		else return w;
	}
	...
	public static int wellToInt(boolean[][] b) {
		...
		for(int i=0; i<width; i++) {
			for(int j=0; j<2; j++) {
				if(b[i][j]) {
					ret += (int)(Math.pow(2,(2*i+j))); //TODO: replace with something more efficient
				}
			}
		}
		return ret;
		...
		for(int gameNo=0;gameNo<totalGames;gameNo++) {
			...
			for(int blockNo=0;blockNo<10000;blockNo++) {
				...
				for(int i=0;i<values.length;i++) {
					if (values[i]>values[bestOption]) bestOption=i;
				}
				...
				double updatedValue = stateValues[wellIndex]*(1-alpha) +
						(reward*rowsRemovedNow + gamma*stateValues[wellToInt(nextWell)])*alpha;
				stateValues[wellIndex] = updatedValue;
				System.out.println("\nGame number " + gameNo + ". The updated value of state " + wellIndex + " is " + updatedValue);
				g.updateWell(nextWell);
				g.updateRows(rowsRemovedNow);
				System.out.println("Rows height now is " + g.rowsHeight + " and the height increased by " + rowsRemovedNow );
